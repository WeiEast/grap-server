 ...
 */
@Component("callbackSuccessMonitorComponent")
public class CallbackSuccessMonitorComponent extends BaseBusinessComponent<OperatorMonitorSpecialRequest> {
    private final static Logger logger = LoggerFactory.getLogger(CallbackSuccessMonitorComponent.class);
    ...
    protected void doBusiness(OperatorMonitorSpecialRequest request) {
        if (request == null || request.getTask() == null) {
            logger.error("运营商监控,发送回调成功(任务日志)消息到monitor,请求参数为空,request={}", JSON.toJSONString(request));
            return;
        }
        if (request.getOrder() == null || request.getOrder() != 6) {
            logger.error("运营商监控,发送回调成功(任务日志)消息到monitor,存在环节遗漏,后续不再执行,request={}", JSON.toJSONString(request));
            return;
        }
        List<TaskLog> list = taskLogService.queryTaskLog(request.getTaskId(), ETaskStep.CALLBACK_SUCCESS.getText());
        if (CollectionUtils.isEmpty(list)) {
            logger.info("运营商监控,发送回调成功(任务日志)消息到monitor,未查询到回调成功日志信息,request={}", JSON.toJSONString(request));
            return;
        }
