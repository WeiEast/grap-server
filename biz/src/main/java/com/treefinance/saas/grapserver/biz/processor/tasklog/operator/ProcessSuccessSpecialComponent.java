 ...
 */
@Component("processSuccessSpecialComponent")
public class ProcessSuccessSpecialComponent extends BaseBusinessComponent<OperatorTaskLogSpecialRequest> {
    private final static Logger logger = LoggerFactory.getLogger(ProcessSuccessSpecialComponent.class);
    ...
    protected void doBusiness(OperatorTaskLogSpecialRequest request) {
        ...
        //收到的msg不是数据保存成功,不处理.
        if (!StringUtils.equalsIgnoreCase(request.getMsg(), ETaskStep.DATA_SAVE_SUCCESS.getText())) {
            return;
        }
